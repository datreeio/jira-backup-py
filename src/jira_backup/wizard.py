...
def create_config():
    ...
    download_locally = input(
        "Do you want to download the backup file locally? (true / false) "
    )
    ...
    if upload_backup.lower() == "true":
        custom_config["UPLOAD_TO_S3"]["AWS_ENDPOINT_URL"] = input(
            "What is your AWS endpoint url? "
        )
        custom_config["UPLOAD_TO_S3"]["AWS_REGION"] = input("What is your AWS region? ")
        custom_config["UPLOAD_TO_S3"]["S3_BUCKET"] = input(
            "What is the S3 bucket name? "
        )
        custom_config["UPLOAD_TO_S3"]["S3_DIR"] = input(
            "What is the S3 directory for upload? (example Atlassian/) "
        )
        custom_config["UPLOAD_TO_S3"]["AWS_ACCESS_KEY"] = input(
            "What is your AWS access key? "
        )
        custom_config["UPLOAD_TO_S3"]["AWS_SECRET_KEY"] = input(
            "What is your AWS secret key? "
        )
        custom_config["UPLOAD_TO_S3"]["AWS_IS_SECURE"] = input(
            "Do you want to use SSL? (true / false) "
        )
